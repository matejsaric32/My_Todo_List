...
class UpdateBoardActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding?.sivBoardImageUpdateBoard?.setOnClickListener {
            ...
            pictureDialog.setItems(pictureDialogItems) {
                    dialog, which ->
                when (which) {
                    0 -> choosePhotoFromGallary() /** Calls the function to choose picture from gallery */
                    1 -> takePhotoFromCamera() /** Calls the function to choose picture form camera */
                }
            }
            pictureDialog.show()
        }
    ...
    private fun setupActionBar() {
        ...
        if (supportActionBar != null) {
            ...
            binding?.toolbarUpdateBoardActivity?.setNavigationIcon(R.drawable.ic_baseline_arrow_back_ios_24)
        }
